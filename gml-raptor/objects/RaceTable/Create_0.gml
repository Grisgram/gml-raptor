...
set_table = function(table_name) {
	if (!race_table_exists(table_name)) {
		if (DEBUG_LOG_RACE)
			elog($"{MY_NAME} could not find race table '{table_name}'. Make sure, it is loaded or check room instance creation order. RaceController must be instantiated first!");
		return;
	}
	
	if (DEBUG_LOG_RACE)
		dlog($"{MY_NAME} received race table to use: raceTable='{table_name}';");
